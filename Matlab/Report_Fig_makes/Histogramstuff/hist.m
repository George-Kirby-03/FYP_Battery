...
histogram(features.minimisetemp.x3,14,'Normalization','probability')
histogram(features.minimisetempmax.x3,14,'Normalization','probability')
...
x = ["Baseline"; "Min Temp"; "Min MaxTemp"; "Min Qloss-Paing"];
figure()
...
bar(x,y)
lb = legend('Averaged Overpotential', 'Maximum Overpotential');
set(lb, 'Interpreter', 'latex');
ylabel('Voltage (V)');
...
x_bold = strcat('\textbf{', string(x), '}');
lb = legend(x_bold);
set(lb, 'Interpreter', 'latex', 'FontSize', 12); 
title('\textbf{Histogram / Temperature binned density plots}', ...
       'interpreter','latex','fontsize',f_size)
grid on
xlabel("$C^\circ$",'interpreter','latex','FontSize',15)
