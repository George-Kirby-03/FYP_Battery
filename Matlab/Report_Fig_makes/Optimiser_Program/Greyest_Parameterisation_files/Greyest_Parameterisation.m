function sim_handler = Greyest_Parameterisation(sim_handler)

%Initial Guess values (also used as check weather to run greyest_


tt = sim_handler.original_data.ts;
tp = sim_handler.original_data.tp;
u1 = sim_handler.original_data.amps;
...
%Creating the table entry for the iddata for the model
t = linspace(0,tt(end),2000);

%Q_heat = (voltage_model - OCV_x).*current_lut(t);
Q_heat = (R0 + R1)*current_lut(t).^2;
...
init_sys = idgrey(@TempModel,parameters,'c');
...
sim_handler.current_sol.Cp = Cp;
sim_handler.current_sol.h = h;
