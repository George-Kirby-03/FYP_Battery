...
%%
t_lin = linspace(0,Cycle10.TestTime(end)-Cycle10.TestTime(1),1000);
hA = 64.9964;
mcp=  0.0991;

model = ss(TempFnc(mcp,hA,0,0));
...
Q_heat = (p.r1+p.r0).*current_lut(t_lin).^2;

simulated_temp = lsim(model, Q_heat,t_lin);
plot(t_lin,simulated_temp)
hold on
