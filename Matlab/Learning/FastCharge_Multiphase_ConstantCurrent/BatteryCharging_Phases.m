function [problem,guess] = BatteryCharging_Phases(problem_mp, guess_mp,i,x0ul,xful,Temp_Max)
...
InternalDynamics=@BatteryCharging_Dynamics_AllPhases;
SimDynamics=[];
...
% Settings file
problem.settings=@settings_BatteryCharging_AllPhases;

% Initial time. t0<tf
problem.time.t0_idx=i;
...
problem.states.x0l=x0ul{i}(1,:); 
problem.states.x0u=x0ul{i}(2,:); 



% State bounds. xl=< x <=xu
problem.states.xl=[0 0 0];
problem.states.xu=[1 0.25 Temp_Max];
...
problem.states.xfl=xful{i}(1,:);
problem.states.xfu=xful{i}(2,:);
...
% Bounds for boundary constraints bl =< b(x0,xf,u0,uf,p,t0,tf) =< bu
% Enforce total charging time ONLY in the final phase

% if i == problem_mp.data.N_phases
%     % Final phase: total time = 600 s
%     problem.constraints.bl = 600;
%     problem.constraints.bu = 600;
% else
%     % Other phases: just positive phase duration
