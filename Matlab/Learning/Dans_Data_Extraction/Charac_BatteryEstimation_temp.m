...
% p=[poly Q C1 R0 R1]
problem.parameters.pl=[problem.data.poly.xl 1.3*3600 400 0.009 0.003 0.01 60];
problem.parameters.pu=[problem.data.poly.xu 1.7*3600 9000 0.08 0.08 3 150];
guess.parameters=[problem.data.poly.xe 1.5*3600 1570 0.026 0.04 0.1 120];
...
problem.states.x0u=[0.7 1 0]; 

% State bounds. xl=< x <=xu
problem.states.xl=[0 -1 -0.5];
problem.states.xu=[1 1 4];
...
problem.states.xfl=[0.5 -1 0];
problem.states.xfu=[1 1 7];

% Guess the state trajectories with [x0 xf]
guess.states(:,1)=[0.3 0.3];
guess.states(:,2)=[0.5 0.5];
guess.states(:,3)=[0 0];
...
stageCost = 0.95*(voltage_model-voltage_measured).^2 + 0.02*(temp_model-temp_measured).^2 + 0.03*(coef-3.65).^2;
