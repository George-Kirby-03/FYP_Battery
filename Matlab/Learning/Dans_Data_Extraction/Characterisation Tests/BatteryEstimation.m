function [problem,guess] = BatteryEstimation(cycle_file)
...
% OCV Poly values
polycount = 7;
problem.data.poly = polymaker(polycount,550,2.5,1,1,1);
% Extract columns
...
% p=[poly Q C1 R0 R1]
problem.parameters.pl=[problem.data.poly.xl 1.5*3600 290 0.07 0.07];
problem.parameters.pu=[problem.data.poly.xu 1.56*3600 330 0.09 0.09];
guess.parameters=[problem.data.poly.xe 1.52*3600 310 0.08 0.08];
...
problem.states.x0l=[0.3 0]; 
problem.states.x0u=[0.4 0]; 

% State bounds. xl=< x <=xu
problem.states.xl=[0 -0.09];
problem.states.xu=[1 0.09];
...
problem.states.xfl=[0.1 0];
problem.states.xfu=[0.8 0];

% Guess the state trajectories with [x0 xf]
guess.states(:,1)=[0.4 0.5];
guess.states(:,2)=[0 0];
...
coef = p(:,1)+p(:,2)+p(:,3)+p(:,4)+p(:,5)+p(:,6)+p(:,7);
stageCost = 0.9*(voltage_model-voltage_measured).^2  + 0.1*(coef-3.6).^2;
