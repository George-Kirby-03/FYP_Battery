...
fun = @(x)sum(zfinal/kmax*p.CellCap./x(1:4:(4*kmax-3),1));
...
for n = 1:noCycles
    ...
    %tic
    [x,fval,exitflag] = fmincon(fun,x0,P,Q,Peq,Qeq,[],[],@nonlconT_vart_noTimeCon,options);
    %toc
...
axis([0,tmax,min(Vsim),3.65])
yyaxis right
axis([0,tmax,0,ceil(max(isim))])
grid on

%% Plot Temperature graphs
TPplot(tsim,Tsim,Pgensim)
yyaxis left
plot([0,tmax],[max(Tsim),max(Tsim)],'--','Color','#EDB120','linewidth',1.5)
grid on
yyaxis right
axis([0,tmax,0,ceil(max(Pgensim)*10)/10])
...
ylabel('Time To Charge (s)','fontSize',16,'interpreter','latex')
ylim([400,2000])