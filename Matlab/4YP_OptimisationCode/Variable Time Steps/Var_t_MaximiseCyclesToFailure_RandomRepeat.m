...
w = load("weights.csv");
C = zeros(4*kmax,1);
C(1:4:(4*kmax-3),1) = w(1:4);
C(4:4:(4*kmax),1) = w(5:8);
fun = @(x)-C'*x;
...
%plot([0,600],[30+dTmax,30+dTmax],'--','Color','#EDB120','linewidth',1.5)
grid on
yyaxis right
...
boxchart(w(9)-ObjectiveFuncVector)
ylabel('Cycles To Failure','fontSize',16,'interpreter','latex')